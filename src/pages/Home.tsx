        ...
        <div className="flex min-h-screen bg-nexus-dark">
            ...
            <main className="flex-1 p-8">
                <header className="mb-10">
                    ...
                    <p className="text-slate-500 text-sm">Bem-vindo ao Nexus, Rian.</p>
                </header>

                {/* INDICADORES*/}
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {mockIndicators.map((item) => (
                        <div key={item.id} className="bg-[#0b0c21]/50 p-6 rounded-2xl border border-white/5 hover:bg-[#0b0c21]/80 transition-all cursor-default">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-10">
                    ...
                    <section className="lg:col-span-2 bg-[#0b0c21]/30 rounded-2xl p-6 border border-white/5 ">
                        ...
                        <div className="space-y-4">
                            {mockTransactions.slice(0, showAll ? undefined : 5).map((transaction) => (
                                <div key={transaction.id} className="flex justify-between items-center p-4 bg-white/5 rounded-xl border border-white/10">
                                    <div>
                                        <p className="text-white font-medium">{transaction.userName}</p>
                                        <p className="text-slate-400 text-xs">{new Date(transaction.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                                    </div>
